...
pub struct MultiSet<K: Eq + Hash> {
    element_counts: std::collections::HashMap<K, usize>,
}
    ...
    K: Eq + Hash,
{
    ...
    pub fn contains(&self, value: &K) -> bool {
        self.element_counts.contains_key(value)
    }
    ...
    pub fn insert(&mut self, value: K) {
        let count = self.element_counts.entry(value).or_insert(0);
        *count += 1;
    }
    ...
    pub fn remove(&mut self, value: &K) -> bool {
        match self.element_counts.get_mut(value) {
            Some(count) => {
                if *count == 1 {
                    self.element_counts.remove(value);
                    true
                } else {
                    *count -= 1;
                    true
    ...
    pub fn count(&self, value: &K) -> usize {
        *self.element_counts.get(value).unwrap_or(&0)
    }
}
